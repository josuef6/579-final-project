...
// Directions used for player and Computer Player movements
let DIRECTION = {
  IDLE: 0,
  UP: 1,
  DOWN: 2,
...
// Sounds used throughout the game: ball-paddle contact, scoring point, winner and loser
let ballHit = new Audio('music/GUI_Sound Effects_031.mp3');
let pointScored = new Audio('music/GUI_Sound Effects_040.mp3');
let playerWins = new Audio('music/MESSAGE-B_Accept.wav');
let playerLoses = new Audio('music/GUI_Sound Effects_033.mp3');

// Creates ball for game
let Ball = {
...
let Game = {
  ...
  // Update Player,  Computer Player, ball movements and keeps track of score
  update: function () {
    if (!this.over) {
      ...
      // Randomly places ball to the correct side for each serve with
      ...
      // Handle Computer Player UP and DOWN movement
      ...
      // Handle Computer Player wall collision
      ...
      // Handle Computer Player-ball collision
      if (
  ...
  draw: function () {
    ...
    // Set the color to Michigan blue
    this.context.fillStyle = this.color;

    // Draw the background
    this.context.fillRect(0, 0, this.canvas.width, this.canvas.height);

    // Set colors for the paddles and the ball
    this.context.fillStyle = "#ffffff";
    ...
    // Draw the Computer Player score (right)
    this.context.fillText(
  ...
  listen: function () {
    document.addEventListener("keydown", function (key) {
      // Handles the start of game by pressing any key on keyboard
      if (Pong.running === false) {
        Pong.running = true;
        window.requestAnimationFrame(Pong.loop);
      }
      // Handle paddle movement for keyboard
    ...
    // Prevents player from moving if no keys are pressed
